...
import academicObserverRoutes from "./routes/academic-observer-routes";
import registerInformeRoutes from "./routes/informe-routes";
import { registerGroupStatsRoutes } from "./routes/group-stats-routes";
import { registerValidationRoutes } from "./routes/validar";
...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Aplicar middleware para modo de desarrollo antes que cualquier otro middleware
  app.use(devModeAuthMiddleware);
  
  // Configurar autenticación primero para tener acceso a los middlewares
  const { verifyToken, checkRole } = setupAuth(app);
  
  // Registrar rutas de validación pública (sin autenticación) - movido más abajo
  
  // Modo desarrollo para bypass de auth en rutas de IA
  const DEV_MODE = process.env.NODE_ENV !== "production";
  
  if (DEV_MODE) {
    console.log("[DEV_MODE] Registrando rutas de IA con bypass de autenticación para desarrollo");
    // En desarrollo, no aplicamos middleware de autenticación para rutas de IA
    app.use(`${API_PREFIX}/ai`, aiRoutes);
    app.use(`${API_PREFIX}/ai`, aiRiskRoutes);
  } else {
    // En producción, aplicamos middleware de autenticación normal
    console.log("[PROD_MODE] Registrando rutas de IA con autenticación completa");
  ...
  // Registrar rutas del módulo AcademicObserver
  app.use(`${API_PREFIX}/academic-observer`, academicObserverRoutes);
  
  // Registrar rutas de estadísticas de grupo
  registerGroupStatsRoutes(app);
  
  // Registrar rutas de validación pública (sin autenticación)
  ...
  // Nueva API segura para estado de recomendaciones IA por lotes - ADMIN ONLY
  app.get(`${API_PREFIX}/ia/batch-status`, verifyToken, async (req, res) => {
    try {
      const requestingUser = (req as any).user;
      
      // Solo administradores pueden acceder
      if (requestingUser.rol !== 'admin') {
        console.log(`[UNAUTHORIZED] Usuario ${requestingUser.userId} (${requestingUser.rol}) intentó acceder a estado de lotes IA`);
        return res.status(403).json({ message: "Solo administradores pueden acceder a esta función" });
      }

      console.log(`[ADMIN] Consultando estado de recomendaciones IA por lotes por usuario ${requestingUser.userId}`);
      
      const batchStatus = await storage.getIABatchStatus();
      
      res.json(batchStatus);
    } catch (error) {
      console.error('❌ Error al obtener estado de lotes IA:', error);
      res.status(500).json({ 
        error: "Error interno del servidor al obtener el estado de lotes IA" 
      });
    }
  });

  // Nueva API segura para generación por lotes simulada - ADMIN ONLY
  app.post(`${API_PREFIX}/ia/batch-generate`, verifyToken, async (req, res) => {
    try {
      const requestingUser = (req as any).user;
      
      // Solo administradores pueden acceder
      if (requestingUser.rol !== 'admin') {
        console.log(`[UNAUTHORIZED] Usuario ${requestingUser.userId} (${requestingUser.rol}) intentó acceder a generación por lotes IA`);
        return res.status(403).json({ message: "Solo administradores pueden acceder a esta función" });
      }

      console.log(`[ADMIN] Iniciando generación por lotes simulada por usuario ${requestingUser.userId}`);
      
      const result = await storage.generateBatchIARecommendationsSimulated(requestingUser.correo || requestingUser.userId);
      
      res.json(result);
    } catch (error) {
      console.error('❌ Error al generar recomendaciones por lotes:', error);
      res.status(500).json({ 
        error: "Error interno del servidor al generar recomendaciones por lotes" 
      });
    }
  });
  ...
  // Endpoint para regenerar un resumen académico con IA (nuevo análisis)
  app.post(`${API_PREFIX}/students/:id/regenerate-summary`, verifyToken, async (req, res) => {
    try {
      const studentId = parseInt(req.params.id);
      ...
      console.log("[DEBUG] Regenerando resumen académico con IA para el estudiante:", estudiante.nombreCompleto);
      
      // Generar nuevo resumen con IA (con una temperatura más alta para mayor variación)
      const resumen = await generarResumenAcademicoParaPadres(datosParaIA, 0.8);
      ...
      console.error("Error al regenerar resumen académico:", error);
      return res.status(500).json({ 
        success: false,
        message: "Error al regenerar resumen académico",
        error: error.message
      });
    }
  });
        ...
        const periodAverages = Object.entries(gradesByPeriod).map(([period, grades]) => {
          ...
          const formattedGrades = Object.entries(gradesByCriteria).map(([criterioId, grade]) => {
            ...
            return {
              ...
              rubro: criterio ? criterio.nombre : (grade.criterioId === 0 ? 'Calificación General' : 'Desconocido'), // Nombre del criterio
  ...
  // Ruta para obtener la configuración de recargos por mora
  app.get(`${API_PREFIX}/institution/late-fees-config`, verifyToken, async (req, res) => {
    try {
      console.log("🔧 Obteniendo configuración de recargos por mora...");
      
      const result = await query(`
        SELECT clave, valor, tipo 
        FROM configuracion_institucional 
        WHERE clave IN ('recargo_habilitado', 'porcentaje_recargo_mora')
      `);
      
      const config = {
        recargoHabilitado: false,
        porcentajeRecargoMora: 10
      };
      
      result.rows.forEach(row => {
        if (row.clave === 'recargo_habilitado') {
          config.recargoHabilitado = row.valor === 'true';
        } else if (row.clave === 'porcentaje_recargo_mora') {
          config.porcentajeRecargoMora = parseFloat(row.valor) || 10;
        }
      });
      
      console.log("✅ Configuración de recargos obtenida:", config);
      res.json(config);
    } catch (error) {
      console.error("❌ Error al obtener configuración de recargos:", error);
      res.status(500).json({ 
        message: "Error al obtener configuración de recargos", 
        error: error instanceof Error ? error.message : String(error)
      });
    }
  });
  ...
  // ----- NOTIFICACIONES -----
  // GET /api/notifications - Obtener notificaciones del usuario (versión estable)
  app.get(`${API_PREFIX}/notifications`, verifyToken, (req, res) => {
    try {
      const user = (req as any).user;
      console.log(`🔐 Usuario autenticado accediendo a notificaciones: ${user.id} (${user.rol})`);
  ...
  // Rutas para informes web (accesibles sin token para enlaces compartibles)
  app.use(`${API_PREFIX}/informe`, registerInformeRoutes());
  ...
  // Registrar rutas de informe académico para padres (sin autenticación)
  app.use(`${API_PREFIX}/informe`, registerInformeRoutes());

  // ===== RUTAS DE COMUNICACIÓN (CON AUTENTICACIÓN) =====
  
  // GET /api/mensajes - Obtener mensajes del usuario autenticado
  app.get(`${API_PREFIX}/mensajes`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      console.log(`🔐 Usuario autenticado accediendo a mensajes: ${user.id} (${user.rol})`);
      
      // Por ahora retornamos un array vacío para evitar errores 401
      // En el futuro se implementará la lógica completa de mensajes
      const mensajes = [];
  ...
  // POST /api/mensajes - Enviar nuevo mensaje (autenticado)
  app.post(`${API_PREFIX}/mensajes`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      const { receiverId, subject, body, priority } = req.body;
      
      console.log(`🔐 Usuario ${user.id} enviando mensaje a ${receiverId}`);
      
      // Por ahora confirmamos recepción sin procesar
      // En el futuro se implementará la lógica completa de envío
      const nuevoMensaje = {
        id: `msg-${Date.now()}`,
        senderId: user.id,
        receiverId,
  ...
  // GET /api/usuarios - Obtener lista de usuarios para comunicación (autenticado)
  app.get(`${API_PREFIX}/usuarios`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      console.log(`🔐 Usuario autenticado accediendo a lista de usuarios: ${user.id} (${user.rol})`);
      
      // Obtener usuarios del sistema para comunicación
      const allUsers = await storage.getUsers();
      
      // Filtrar usuarios según el rol del solicitante
      const usuariosPermitidos = allUsers
  ...
  // GET /api/calendar - Obtener eventos del calendario (autenticado)
  app.get(`${API_PREFIX}/calendar`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      console.log(`🔐 Usuario autenticado accediendo a calendario: ${user.id} (${user.rol})`);
      
      // Por ahora retornamos un array vacío para evitar errores 500
      // En el futuro se implementará la lógica completa del calendario
      const events = [];
  ...
  // Public validation routes are registered above with registerValidationRoutes(app)

  // Ruta pública para descargar informes académicos asociados a recibos
  app.get('/api/informes/download/:id', async (req, res) => {
    try {
      ...
      const recibo = await storage.getPayment(parseInt(receiptId));
      if (!recibo) {
        return res.status(404).json({ message: "Recibo no encontrado" });
      }

      // Construir la ruta del informe académico
      const informePath = path.join(process.cwd(), 'public', 'informes', `informe_${receiptId}.pdf`);
      
      try {
        // Verificar que el archivo existe
        await fs.access(informePath);
        
        // Obtener información del estudiante para el nombre del archivo
        const estudiante = await storage.getStudent(recibo.alumnoId);
        const studentName = estudiante ? estudiante.nombreCompleto.replace(/\s+/g, '_') : 'Estudiante';
        
        // Configurar headers para descarga
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="Informe_Academico_${studentName}_${receiptId}.pdf"`);
        res.setHeader('Cache-Control', 'no-cache');
        
        // Leer y enviar el archivo
        const fileBuffer = await fs.readFile(informePath);
        res.send(fileBuffer);
        
        console.log(`📥 Informe académico descargado: recibo ${receiptId} para ${studentName}`);
        
      } catch (accessError) {
        return res.status(404).json({ 
          message: "Informe académico no disponible para este recibo",
          receiptId: receiptId
        });
      }
      
    } catch (error) {
      console.error('❌ Error descargando informe académico:', error);
      res.status(500).json({ message: "Error interno del servidor" });
    }
  });

  // ============================================
  // RUTAS PARA HISTORIAL ACADÉMICO (PORTAL PADRES)
  // ============================================

  // Obtener estudiantes vinculados a un padre
  app.get(`${API_PREFIX}/parent/students`, verifyToken, async (req, res) => {
    try {
      const parentUserId = (req as any).user.id;
      
      // Obtener estudiantes vinculados al padre actual
      const students = await storage.getStudentsByParent(parentUserId);
      
      if (!students || students.length === 0) {
        return res.json([]);
      }

      // Formatear respuesta con información básica del estudiante
      const formattedStudents = students.map(student => ({
        id: student.id,
        nombreCompleto: student.nombreCompleto,
        grado: student.nivel || 'N/A',
        grupo: student.grupoId ? `Grupo ${student.grupoId}` : 'N/A',
        activo: student.estatus === 'activo'
      }));

      res.json(formattedStudents);
    } catch (error) {
      console.error('Error al obtener estudiantes del padre:', error);
      res.status(500).json({ message: "Error al obtener estudiantes vinculados" });
    }
  });

  // Endpoint RESTful para obtener estudiantes del padre autenticado (para el selector IA)
  app.get(`${API_PREFIX}/parent/me/students`, verifyToken, async (req, res) => {
    try {
      const parentUserId = (req as any).user.id;
      
      console.log(`[PARENT/ME/STUDENTS] Obteniendo estudiantes para padre: ${parentUserId}`);
      
      // Obtener estudiantes vinculados al padre actual
      const students = await storage.getStudentsByParent(parentUserId);
      
      if (!students || students.length === 0) {
        console.log(`[PARENT/ME/STUDENTS] No se encontraron estudiantes para el padre: ${parentUserId}`);
        return res.json([]);
      }

      // Formatear respuesta optimizada para el selector de estudiantes
      const formattedStudents = students.map(student => ({
        id_alumno: student.id,
        nombre: student.nombreCompleto,
        nivel: student.nivel || 'N/A',
        matricula: `EST-${student.id.toString().padStart(4, '0')}`
      }));

      console.log(`[PARENT/ME/STUDENTS] Retornando ${formattedStudents.length} estudiantes`);
      res.json(formattedStudents);
    } catch (error) {
      console.error('[PARENT/ME/STUDENTS] Error al obtener estudiantes:', error);
      res.status(500).json({ 
  ...
  app.get(`${API_PREFIX}/parent/academic-history/:studentId`, verifyToken, async (req, res) => {
    try {
      const parentUserId = (req as any).user.id;
      const studentId = parseInt(req.params.studentId);
      
      console.log(`🔍 DEBUG: Verificando relación entre estudiante ${studentId} y padre ${parentUserId}`);
      
      // Verificar que el estudiante pertenezca al padre autenticado
      const studentBelongsToParent = await storage.verifyStudentParentRelation(studentId, parentUserId);
      
      if (!studentBelongsToParent) {
        console.log(`❌ DEBUG: Sin permisos para estudiante ${studentId}`);
        return res.status(403).json({ 
          message: "No tiene permisos para acceder al historial de este estudiante" 
        });
      }

      console.log(`✅ DEBUG: Relación encontrada`);

      // Obtener información del estudiante
      const student = await storage.getStudent(studentId);
      if (!student) {
        console.log(`❌ DEBUG: Estudiante ${studentId} no encontrado`);
        return res.status(404).json({ message: "Estudiante no encontrado" });
      }

      console.log(`✅ DEBUG: Alumno encontrado: ${student.nombreCompleto}, nivel: ${student.nivel}, estatus: ${student.estatus}`);

      // Obtener informes académicos directamente de la base de datos
      console.log(`🔍 DEBUG: Consultando informes académicos para estudiante ${studentId}`);
      
      const academicReports = await storage.getAcademicReportsByStudent(studentId);
      
      console.log(`📊 DEBUG: Encontrados ${academicReports.length} informes académicos`);

      res.json(academicReports);
  ...
  // Verificar existencia y accesibilidad de un informe académico específico
  app.head(`${API_PREFIX}/parent/academic-report/:studentId/:reportId`, verifyToken, async (req, res) => {
    try {
      const parentUserId = (req as any).user.id;
      const studentId = parseInt(req.params.studentId);
      ...
      const informePath = path.join(process.cwd(), 'public', 'informes', `informe_${reportId}.pdf`);
      
      try {
        await fs.access(informePath);
        const stats = await fs.stat(informePath);
      ...
      console.error('Error al verificar informe académico:', error);
      res.status(500).end();
    }
  });

  // Secure endpoint for predictive AI recommendations
  app.post(`${API_PREFIX}/predict-recommendation`, verifyToken, async (req, res) => {
    try {
      ...
      if (!student_id || typeof student_id !== 'number') {
        return res.status(400).json({ 
          error: "student_id es requerido y debe ser un número" 
        });
      }
      
      console.log(`[AI] Generando recomendación predictiva para estudiante ID: ${student_id}`);
      
      // Check for existing cached recommendation
      const existingRecommendation = await storage.getIaRecommendation(student_id);
      
      if (existingRecommendation) {
        console.log(`[AI] Usando recomendación cacheada para estudiante ${student_id}`);
        return res.json({
          recomendacion_predictiva: existingRecommendation.textoGenerado,
          cached: true,
          fecha_generacion: existingRecommendation.fechaGeneracion
        });
      }
      
      // Get student data from database
      const student = await storage.getStudent(student_id);
      if (!student) {
        return res.status(404).json({ 
      ...
      console.log(`[AI] Generando recomendación predictiva para: ${student.nombreCompleto}`);
      
      // Get student grades to calculate average
      const grades = await storage.getGradesByStudent(student_id);
      const average = grades.length > 0 
        ? grades.reduce((sum, grade) => sum + parseFloat(grade.valor.toString()), 0) / grades.length 
        : 8.5; // Default average for testing
      
      // Predefined strengths and improvement areas based on academic profile
      const fortalezas = [
      ...
      // Generate AI recommendation using Anthropic service
      const { generatePredictiveRecommendation } = await import("./services/anthropic-service");
      ...
      const recomendacion = await generatePredictiveRecommendation(studentData);
      
      // Save recommendation to database for caching
      try {
        const cachedRecommendation = await storage.createIaRecommendation({
          studentId: student_id,
          textoGenerado: recomendacion
        });
        
        console.log(`[AI] Recomendación guardada en cache para ${student.nombreCompleto}`);
        console.log(`[AI] Recomendación generada exitosamente (${recomendacion.length} caracteres)`);
        
        res.json({
          recomendacion_predictiva: recomendacion,
          cached: false,
          fecha_generacion: cachedRecommendation.fechaGeneracion
        });
      } catch (cacheError) {
        console.error("Error guardando en cache, devolviendo recomendación sin cache:", cacheError);
        
        // Return recommendation even if caching fails
        res.json({
          recomendacion_predictiva: recomendacion,
          cached: false
        });
      ...
      res.status(500).json({ 
        error: "Error al generar recomendación predictiva",
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });

  // Secure admin endpoint to view IA recommendations for internal review
  app.get(`${API_PREFIX}/ia-recommendation/:student_id`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      ...
      console.log(`[ADMIN] Consultando recomendación IA para estudiante ${studentId} por usuario ${user.id}`);
      ...
      // Get IA recommendation from cache
      const recommendation = await storage.getIaRecommendation(studentId);
      
      if (!recommendation) {
        return res.status(404).json({ 
          error: "No se encontró recomendación IA para este estudiante",
          student: {
            id: student.id,
            nombre: student.nombreCompleto
          }
      ...
      console.log(`[ADMIN] Recomendación IA encontrada para ${student.nombreCompleto}`);
      ...
      console.error("Error consultando recomendación IA:", error);
      res.status(500).json({ 
        error: "Error al consultar recomendación IA",
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });

  // Phase 6: Teacher IA Recommendations Route
  app.get(`${API_PREFIX}/teacher/ia-recommendations`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      ...
      console.log(`[TEACHER] Consultando recomendaciones IA para docente ${user.correo}`);
      
      // Get teacher's assigned students and their recommendations
      const teacherRecommendations = await storage.getTeacherStudentRecommendations(user.id);
      
      console.log(`[TEACHER] Encontradas ${teacherRecommendations.length} recomendaciones para el docente`);
      
      res.json(teacherRecommendations);
    } catch (error) {
      console.error('Error al obtener recomendaciones del docente:', error);
      res.status(500).json({ 
        error: 'Error interno del servidor',
  ...
  // Phase 6: Parent IA Recommendation Route with student-specific filtering
  app.get(`${API_PREFIX}/parent/ia-recommendation`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      const requestedStudentId = req.query.studentId ? parseInt(req.query.studentId as string) : null;
      
      // Verify parent role
      if (user.rol !== 'padre') {
        return res.status(403).json({ 
          error: 'Acceso denegado',
          message: 'Solo los padres de familia pueden acceder a esta función'
        });
      }

      console.log(`[PARENT] Consultando recomendación IA para padre ${user.correo}, estudiante: ${requestedStudentId || 'cualquiera'}`);
      
      try {
        // Step 1: Validate parent-child relationship
        const studentIds = await storage.getParentStudentIds(user.id);
        console.debug("Hijos encontrados:", studentIds);
        
        if (!studentIds || studentIds.length === 0) {
          return res.status(200).json({
            status: "not_found",
            message: "No se han encontrado hijos asociados a este usuario."
          });
        }
        
        // Step 2: If specific student requested, validate ownership and get their recommendation
        if (requestedStudentId) {
          if (!studentIds.includes(requestedStudentId)) {
            return res.status(403).json({
              status: "not_found",
              message: "No tiene autorización para acceder a los datos de este estudiante."
            });
          }
          
          const recommendation = await storage.getIARecommendationByStudentId(requestedStudentId);
          if (recommendation) {
            const student = await storage.getStudentById(requestedStudentId);
            console.debug(`Recomendación IA encontrada para estudiante específico: ${student?.nombreCompleto || `ID ${requestedStudentId}`}`);
            ...
            return res.status(200).json({
              ...
              recommendation: {
                student: {
                  ...
                  nombre: student?.nombreCompleto || 'Estudiante',
                  nivel: student?.nivel || 'N/A'
                },
                recommendation: {
                  id: recommendation.id,
            ...
            return res.status(200).json({
              status: "not_found",
              message: `No se ha generado una recomendación IA para el estudiante con ID ${requestedStudentId}.`
            });
          }
        }
        
        // Step 3: If no specific student requested, search for any recommendation
        for (const studentId of studentIds) {
          const recommendation = await storage.getIARecommendationByStudentId(studentId);
          
          if (recommendation) {
            const student = await storage.getStudentById(studentId);
            console.debug("Recomendación IA encontrada para:", student?.nombreCompleto || `ID ${studentId}`);
        ...
        // Step 4: No recommendations found for any child
        return res.status(200).json({
          status: "not_found",
          message: "No se ha generado una recomendación IA para ninguno de los hijos asociados a este usuario."
        });
        
      } catch (error) {
        console.error(`[PARENT] Error en consulta:`, error);
        return res.status(200).json({
          status: "not_found",
          message: "Error al consultar las recomendaciones IA."
        });
      }
    } catch (error) {
      console.error('Error al obtener recomendación del hijo:', error);
      res.status(500).json({ 
        error: 'Error interno del servidor',
  ...
  // Phase 4: IA Recommendation History Endpoints
  // Get recommendation history for a student (admin only)
  app.get(`${API_PREFIX}/ia-recommendation/:student_id/history`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      ...
      if (user.rol !== 'admin') {
        return res.status(403).json({ 
          error: "Acceso denegado. Solo administradores pueden ver el historial de recomendaciones" 
        });
      }
      ...
      console.log(`[ADMIN] Consultando historial de recomendaciones IA para estudiante ${studentId} por usuario ${user.id}`);
      ...
      const history = await storage.getIaRecommendationHistory(studentId);
      
      console.log(`[ADMIN] Historial de recomendaciones encontrado: ${history.length} versiones para ${student.nombreCompleto}`);
      ...
      res.json({
        ...
        history: history.map(item => ({
          ...
          fecha_generacion: item.fechaGeneracion,
          tipo_generacion: item.tipoGeneracion,
          modo_generacion: item.modoGeneracion,
          generado_por: item.generadoPor,
          preview: item.contenido.substring(0, 150) + (item.contenido.length > 150 ? '...' : '')
        }))
      });
      
    } catch (error) {
      console.error("Error consultando historial de recomendaciones IA:", error);
      res.status(500).json({ 
        error: "Error al consultar historial de recomendaciones IA",
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });

  // Get full content of a specific recommendation from history (admin only)
  app.get(`${API_PREFIX}/ia-recommendation-history/:history_id`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      ...
      if (user.rol !== 'admin') {
        return res.status(403).json({ 
          error: "Acceso denegado. Solo administradores pueden ver el contenido del historial" 
        });
      }
      ...
      console.log(`[ADMIN] Consultando contenido de historial IA ${historyId} por usuario ${user.id}`);
      
      // Get history item
      const historyItem = await storage.getIaRecommendationHistoryById(historyId);
      ...
      // Get student info
      const student = await storage.getStudent(historyItem.studentId);
      
      console.log(`[ADMIN] Contenido de historial encontrado para estudiante ${student?.nombreCompleto || historyItem.studentId}`);
      ...
      res.json({
        ...
        contenido: historyItem.contenido,
        fecha_generacion: historyItem.fechaGeneracion,
        tipo_generacion: historyItem.tipoGeneracion,
        modo_generacion: historyItem.modoGeneracion,
        generado_por: historyItem.generadoPor
      });
      
    } catch (error) {
      console.error("Error consultando contenido de historial IA:", error);
      res.status(500).json({ 
        error: "Error al consultar contenido de historial IA",
        details: error instanceof Error ? error.message : String(error)
      });
    }
  });

  // Endpoint para generación por lote de recomendaciones IA
  app.post(`${API_PREFIX}/ia-recommendations/batch-generate`, verifyToken, async (req, res) => {
    try {
      const requestingUser = (req as any).user;
      
      // Solo administradores pueden generar por lote
      if (requestingUser.rol !== 'admin') {
        console.log(`[UNAUTHORIZED] Usuario ${requestingUser.id} (${requestingUser.rol}) intentó generar recomendaciones por lote`);
        return res.status(403).json({ message: "Solo administradores pueden generar recomendaciones por lote" });
      }
      ...
      if (!Array.isArray(studentIds) || studentIds.length === 0) {
        return res.status(400).json({ message: "Debe proporcionar al menos un ID de estudiante" });
      }
      
      if (studentIds.length > 5) {
        return res.status(400).json({ message: "No puede generar más de 5 recomendaciones por lote" });
      }

      console.log(`[ADMIN] Iniciando generación por lote para ${studentIds.length} estudiantes por usuario ${requestingUser.id}`);
      ...
      for (const studentId of studentIds) {
        try {
          ...
          if (!student) {
            results.push({
              studentId,
              studentName: `Estudiante ID ${studentId}`,
              success: false,
              error: "Estudiante no encontrado"
            });
            errorCount++;
          ...
          const existingRecommendation = await storage.getIaRecommendation(studentId);
          if (existingRecommendation) {
            results.push({
              studentId,
              studentName: student.nombreCompleto,
              success: false,
              error: "El estudiante ya tiene una recomendación generada"
            });
            errorCount++;
            continue;
          }

          // Generar recomendación usando el método existente
          const recommendation = await storage.getOrCreateIARecommendation(studentId, false);
          ...
          console.error(`❌ Error generando recomendación para estudiante ${studentId}:`, error);
          
          let errorMessage = "Error interno al generar recomendación";
          ...
          results.push({
            studentId,
            studentName: student?.nombreCompleto || `Estudiante ID ${studentId}`,
            success: false,
            error: errorMessage
          });
          errorCount++;
      ...
      console.log(`[BATCH_COMPLETE] Procesados: ${results.length}, Exitosos: ${successCount}, Errores: ${errorCount}`);
      ...
      let statusCode = 500;
      let errorMessage = "Error interno del servidor en generación por lote";
      
      // Manejo específico de errores de Claude API
      if (error instanceof Error) {
        if (error.message.includes('429') || error.message.includes('quota') || error.message.includes('limit')) {
          statusCode = 429;
          errorMessage = "Se ha alcanzado el límite de uso de la IA. Intente más tarde.";
        } else if (error.message.includes('402') || error.message.includes('payment')) {
          statusCode = 402;
          errorMessage = "Error de configuración de la IA. Contacte al administrador técnico.";
        } else if (error.message.includes('503') || error.message.includes('unavailable')) {
          statusCode = 503;
  ...
  // Admin endpoint to regenerate IA recommendation manually
  app.post(`${API_PREFIX}/ia-recommendation/:student_id/refresh`, verifyToken, async (req, res) => {
    try {
      const user = (req as any).user;
      
      console.log(`[DEBUG REFRESH] Usuario autenticado:`, {
        id: user?.id,
        rol: user?.rol,
        correo: user?.correo
      });
      ...
      if (!user || user.rol !== 'admin') {
        console.log(`[DEBUG REFRESH] Acceso denegado - Rol: ${user?.rol || 'undefined'}`);
        return res.status(403).json({ 
          error: "Acceso denegado. Solo administradores pueden regenerar recomendaciones" 
        });
      }
      ...
      console.log(`[ADMIN] Regenerando recomendación IA para estudiante ${studentId} por usuario ${user.id}`);
      ...
      try {
        const recommendation = await storage.getOrCreateIARecommendation(studentId, true);
        
        console.log(`[ADMIN] Recomendación IA regenerada exitosamente para ${student.nombreCompleto}`);
        ...
        console.error(`[ADMIN] Error específico de IA al regenerar:`, aiError);
        
        // Check for Claude API specific errors
        const errorMessage = aiError instanceof Error ? aiError.message : String(aiError);
        
        // Handle quota/credit exhaustion errors
        if (errorMessage.includes('quota') || 
          ...
          return res.status(429).json({
            error: "No se pudo regenerar la recomendación porque se ha alcanzado el límite de uso de la IA. Por favor, contacta al administrador.",
            type: "quota_exceeded"
          });
        }
        ...
        if (errorMessage.includes('unauthorized') || 
          ...
          return res.status(402).json({
            error: "Error de configuración de la IA. Por favor, contacta al administrador técnico.",
            type: "auth_error"
          });
        }
      ...
      console.error(`[ADMIN] Error general al regenerar recomendación IA:`, error);
      res.status(500).json({ 
        error: "Error interno del servidor al regenerar recomendación" 
      });
    }
  });
